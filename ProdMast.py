...
QUBEDRIVE_UPLOAD_WAIT = 2 * 60     # 2 menit, tunggu setelah upload_data
...
START_LOOKBACK_DAYS    = 3   # rentang "promo baru mulai" (hari ini atau s/d 3 hari lalu) -> ikut PRICE ADJUSTMENT
...
def xs_run_qubedrive_upload() -> bool:
    ...
    try:
        subprocess.Popen([QUBEDRIVE_EXE] + QUBEDRIVE_UPLOAD_ARGS)
    ...
    log.info(f"[XShelf] STEP 0-PRE b - Menunggu {QUBEDRIVE_UPLOAD_WAIT // 60} menit setelah upload_data dijalankan...")
    start_wait = time.time()
    while time.time() - start_wait < QUBEDRIVE_UPLOAD_WAIT:
        elapsed_min = int((time.time() - start_wait) // 60)
        remaining_min = (QUBEDRIVE_UPLOAD_WAIT // 60) - elapsed_min
        if elapsed_min > 0 and (time.time() - start_wait) % 60 < 5:
            log.info(f"  ... menunggu upload_data, sisa ~{remaining_min} menit")
        time.sleep(5)
    log.info("  OK: Selesai menunggu upload_data")
    return True
